...
#计算更新算子
    ...
    def Ada_L1MKLSVC_FISTA_(self,):
        ...
        while 1:
            ...
            reg=SVC(C=self.C,gamma=self.gamma,kernel='precomputed',random_state=42).fit(MKL_mat_fin,self.y_train,sample_weight=self.beta[0])
            self.REG.append(reg) 
            self.BETA.append(self.beta) 
            self.D.append(self.d)
            reg_pre=reg.predict(MKL_mat_fin)
            
            #calculate residual error err
            I0=np.zeros([1,self.N])
            I0[0][reg_pre!=self.y_train]=1
